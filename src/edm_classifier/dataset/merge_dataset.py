...
def embedding_difference_metrics(left: Path, right: Path) -> dict[str, float]:
    a = np.load(left, allow_pickle=False).astype(np.float64)
    b = np.load(right, allow_pickle=False).astype(np.float64)
        ...
        if canonical_embedding.exists():
            ...
            if (
                ...
                conflicts.append(
                    {
                        "video_id": video_id,
                        "canonical": str(canonical_embedding),
                        "v2": str(v2_embedding),
                        "resolution": "kept_canonical_v1",
                        **embedding_difference_metrics(
                            canonical_embedding,
                            v2_embedding,
                        ),
                    }
